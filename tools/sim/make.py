			...
			for ipa, info in item[hz]:
				...
				if n > 1:
					...
					if yd == 1 and (info.startswith("白") or info.startswith("(白)")):
						continue
					if info.startswith("又") or info.startswith("(又)"):
						continue
...
def dump_html(dialects):
	import datetime
	now = datetime.datetime.now(tz=datetime.timezone(datetime.timedelta(hours=8)))
	html = open("template.html", "r", encoding="U8").read().replace("{{DIALECTS}}", "\n".join(f"    new Array{item}," for item in dialects)).replace("{{DATE}}", " (" + now.strftime("%Y-%m-%d") + ")")
...
def main():
	...
	for lang, item in dicts.items():
		result = []
		result.extend(get_result(0, 30 * 2, lang, item, index=0))
		result.extend(get_result(30 * 2, 90 * 2, lang, item, index=1))
		result.extend(get_result(90 * 2, 100 * 2, lang, item, index=2))
		result.extend(get_result(0, 30 * 2, lang, item, index=3))
		result.extend(get_result(30 * 2, 90 * 2, lang, item, index=4))
		result.extend(get_result(90 * 2, 100 * 2, lang, item, index=5))
		白讀 = "".join(result[0:100])
		文讀 = "".join(result[100:200])
		if 白讀.count('-') > 20:
