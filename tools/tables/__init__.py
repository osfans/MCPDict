		...
		if mod in 詳情:
			...
			if 語.爲語() and 語.爲音 and 語.字數 < 10000:
				for 字,音 in 語.字音典.items():
					...
					if len(音) > 1 and 字 in "娟娼疸疽妍姘冢汊權杈姓磐罄迂遷亘修侈局屙人入腿服嘖椽橡誦諵領頷藩落林杯戴胃戍戌戎戊柘輻輜裂製來米刺剌已已巳文新訓舊老丕魚白臼吉杳查爹釜青直野墅翼舅篁簟廊癬吟妗妙沮泪淚揈掏掬病橘𣠽裔筒篙蒿嵩整鏊樞慪柩嗩噴纤紝家冢餓饑莽蕎薺揪偢報抿槍桅捕捅肉囱算箅孟盂器囂疔療飄瓢瓤燥爆悼焯壁璧璺哆眵算耵倍許託":
						語.誤.append(f"【{字}】的讀音太多:{','.join(音)}")
			語.info["解析日志"] = None
			語.info["同音字表"] = None
			if 語.誤:
