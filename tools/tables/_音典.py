	...
	def 析(自, 列):
		...
		if 自.列序:
			...
			if 名 in ("信宜新寶",):
				...
				音 = ("%s%s〚%s%s{切}%s{聲}%s%s〛"%(列[12], f"/{列[13]}" if 列[13] else "", 列[7],列[8],列[9],列[10],列[11]))
			elif 名 in ("溫州",):
				...
				音 = 音.replace(".ŋ", "ŋ̍")
				if 音.startswith("ø"): 音 = 音[1:]
			elif 名 in ("臺灣四縣腔", "臺灣海陸腔", "臺灣大埔腔", "臺灣詔安腔", "臺灣饒平腔", "臺灣南四縣腔"):
				音 = 自.臺灣客語音標(音)
				if 字 == "󿕅": 字 = "𦢊"
			elif 名 in ("1600福州",):
				自.爲音 = False
