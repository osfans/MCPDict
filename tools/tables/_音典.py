...
class 表(_表):
	def 析(自, 列):
		...
		音標 = ""
		註 = ""
		if 名 in ("汝城", "瑞安東山", "新界客家話", "長壽", "宜章巖泉","郴州","樂昌皈塘","尤溪","晉江", "龍門路溪", "詔安", "道縣官話", "重慶", "樂昌三溪", "南安", "東方感城","汝城延壽", "建陽連墩","江門荷塘(上)","江門荷塘(下)","惠來隆江", "韶關大村", "韶關向陽", "樂東黃流", "東莞東坑", "壽寧平溪"):
			字, 音, 註 = 列[:3]
		elif 名 in ("通州金沙","華安髙安","五華"):
			音, 字, 註 = 列[:3]
		elif 名 in ("江陰", "江陰新橋", "江陰申港", "信宜新寶", "富陽東梓關", "新登城陽"):
			_, 字, 註, 音 = 列[:4]
			if 字 == 註: 註 = ""
		elif 名 in ("東方八所",):
			_, 字, 聲韻, 調, 註 = 列[:5]
		...
		#音標
			...
			_, 字, 聲, 韻, 調值, 註 = 列[:6]
			音標 = 聲 + 韻 + 調值
		elif 名 in ("淮南","懷遠","鳳陽","陽新國和","上猶", "南陵仙坊", "漢口", "連州", "連州保安", "連州星子", "連州西岸", "連州豐陽","龍游", "上猶社溪"):
			_, 字, 聲, 韻, 調值, _, 註 = 列[:7]
			音標 = 聲 + 韻 + 調值
			音標 = 音標.lstrip("0")
		elif 名 in ("國語",):
			...
			註 = 列[0][1:].strip("()（）")
			音標 = 列[自.音列]
		elif 自.文件名.startswith("安徽西北部五县连表"):
			字 = 列[0]
			音標 = 列[自.音列]
