...
class 表(_表):
	...
	def 統(自, 行):
		...
		if 名 in ("黃梅小池","光山南郊", "信陽東雙河"):
		...
		elif 名 in ("浦城觀前","泰順莒江","龍游靈上","南平洋頭", "蓮花坊樓","延安老戶話","慶元江根","閩淸坂東", "泰順三魁", "蒼南炎亭", "玉山", "常山", "慶元"):
			行 = re.sub(r"(‖)(\[\d+\])", "\\2\\1",行)
			while (newline := re.sub(r"(?<=‖)([^\[\]]*[^‖]){", "\\1‖{", 行)) != 行:
				行 = newline
			行 = re.sub("‖{", "{(連讀音)", 行).replace("‖", "")
		elif 名 in ("福鼎白琳","福淸","福安穆陽", "屛南","古田大橋","古田杉洋", "霞浦長春", "柘榮富溪", "壽寧斜灘"):
			行 = re.sub(r"([\|‖])(\[\d+\])", "\\2\\1",行)
			while (newline := re.sub(r"(?<=‖)([^\[\]]*[^‖]){", "\\1‖{", 行)) != 行:
				行 = newline
			while (newline := re.sub(r"(?<=\|)([^\[\]]*[^\|]){", "\\1|{", 行)) != 行:
			...
			行 = 行.replace("/", "", 2)
		elif 名 in ("烏魯木齊","西寧","蒙山新圩","靑島","天台東鄕","景寧東坑"):
