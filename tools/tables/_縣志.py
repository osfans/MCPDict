...
class 表(_表):
	...
	def 統(自, 行):
		...
		if 名 in ("黃梅小池","光山南郊", "信陽東雙河"):
			...
			行 = 自.normG(行)
			行 =  re.sub(r"‖([^\[\]‖]+)", 自.官話音, 行)
		elif 名 in ("韓城王河","浦城觀前","泰順莒江","龍游靈上","南平洋頭", "蓮花坊樓","延安老户話","慶元江根","閩清坂東", "閩清塔莊", "泰順三魁", "蒼南炎亭", "玉山", "常山", "慶元", "廣豐", "江山","景寧鄭坑","南平峽陽", "建陽社州", "崇安五夫", "安吉郎村", "平陽溪南", "縉雲廷坑", "開化龍門", "常山西源", "象山爵溪", "當塗湖陽", "開化華埠", "金華珊瑚", "大田廣平"):
			行 = re.sub(" ?‖ ?", "‖", 行)
			行 = re.sub(r"(‖)(\[\d+\])", "\\2‖",行)
			行 = re.sub(r"\[(\D.*?)\]", "［\\1］",行)
			行 = 自.normG(行)
