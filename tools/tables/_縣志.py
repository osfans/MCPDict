...
class 表(_表):
	...
	def 連讀12(m):
		r = ""
		mark = m.group(1)
		tag = "連讀音" + ("Ⅰ" if mark == "|" else "Ⅱ")
		for i,j in re.findall("(.)(\\d?[-=?]?｛.*?｝)?", m.group(2).strip()):
			if "｛" in j: j = j.replace("｛",f"｛({tag})", 1)
			else: j += f"｛{tag}｝"
			r += i + j
		return r
	...
	def 統(自, 行):
		...
		if 名 in ("黃梅小池","光山南郊", "信陽東雙河"):
			...
			行 = re.sub(r"(\d+)(?![:\d])", "[\\1]", 行)
		elif 名 in ("美斯樂果敢話", "美斯樂瀾滄話", "美斯樂騰衝話", "北京", "德安", "興國", "興國茶園", "永新", "安遠", "廉州", "賀州鋪門", "賀州靈鳳", "賀州擔石", "梧州倒水", "柳城古砦", "融安", "孝昌小河", "永福桃城", "瑞金", "上杭藍溪", "平樂船上話", "富川秀水", "松江", "果敢"):
			上標 = "⁰¹²³⁴⁵⁶⁷⁸⁹"
			...
			行 = 自.normG(行)
			行 =  re.sub(r"‖([^\[\]‖]+)", 自.官話音, 行)
		elif 名 in ("韓城王河","浦城觀前","泰順莒江","龍游靈上","南平洋頭", "蓮花坊樓","延安老户話","慶元江根","閩清坂東", "閩清塔莊", "泰順三魁", "蒼南炎亭", "玉山", "常山", "慶元", "廣豐", "江山","景寧鄭坑","南平峽陽", "建陽社州", "崇安五夫", "安吉郎村", "平陽溪南", "縉雲廷坑", "開化龍門", "常山西源", "象山爵溪", "當塗湖陽", "開化華埠", "金華珊瑚"):
			行 = re.sub(" ?‖ ?", "‖", 行)
			行 = re.sub(r"(‖)(\[\d+\])", "\\2‖",行)
			行 = re.sub(r"\[(\D.*?)\]", "［\\1］",行)
			行 = 自.normG(行)
			...
			if 行.startswith("\t"): 行 = ""
		elif 名 in ("丹鳳","嘉定中","嘉定西","嘉定城","嘉定華亭","寶山羅店","南皮","海門","長治", "鄱陽","萊西前張管寨"):
			if 行.startswith("#"): 行 = "#"
		elif 名 in ("新祁陽白水",):
			if 行.startswith("#"): 行 = "#"
			行 = 行.replace("[213]", "[214]").replace("[224]", "[214]").replace("[232]", "[22]").replace("[442]", "[44]")
		elif 名 in ("商洛",):
			if 行.startswith("#"): 行 = "#"
			行 = re.sub(r"\[([^\d]+)\]", "\\1", 行)
			...
			行 = 行.replace("\\n", "")
		elif 名 in ("南通唐閘","如皋白蒲","南通小海","南通新開","興化戴南","啟東", "廣陵", "新南京", "如東兵房"):
			if 行.startswith("\t"):
				return
			行 = 自.增加調類(行)
			行 = 自.normS(行.replace(")(", "："))
	...
	def 析韻(自, 行):
		...
		if 有字(韻):
			...
			return 自.韻
		if not sharp and 自.簡稱 in ("平樂石龍廠","揭西灰寨","福鼎", "巫山", "水富","石棉", "天全", "廣安", "杭州塘棲", "餘杭", "杭州臨平","沙市"):
			return 自.韻
	...
	def 更新(自):
		...
		for 行 in open(自.spath,encoding="U8"):
			...
			for 行 in 統行.split("\n"):
				...
				列[1] = re.sub(r"｛[^｛｝]*?｝", lambda x:x.group(0).replace("［", "[").replace("］", "]"), 列[1])
				聲, 字組 = 列
				if 韻 != "" and 聲 == "ø": 聲 = ""
				名 = 自.簡稱
