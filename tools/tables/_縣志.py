...
class 表(_表):
	...
	def 統(自, 行):
		...
		if 名 in ("永州嵐角山", "賀州南鄕", "松江天馬", "運城", "興縣","豐城","豐城鐵路","新建","賀州江坪"):
			...
			行 = 行.replace("ø[", "0[")
		elif 名 in ("遂川","大庸南","大庸北", "婺川", "蒙山程村","欽州東場", "陽朔鳳樓","桑植芙蓉橋", "大庾", "沿河","武威","永修","澄海大新","光山斛山", "南康唐江", "仁化長江", "永豐", "南豐","崇左大新","商州北寬坪","耒陽", "大豐三龍","涼城", "淄川", "道眞", "子洲馬蹄溝", "鹽池花馬池", "鹽山", "南召", "南召白土崗", "漢中龍江", "南鄭大河坎", "全州石塘", "興安髙尙","咸陽","射洪靑崗","蓬溪天福","靈寶陽平", "右玉", "保德"):
			行 = 自.行轉調類(行)
		elif 名 in ("奉化",):
			行 = 自.行轉調類(行, r"(\d+)(?![:\d])")
		elif 名 in ("黃梅小池","光山南郊"):
