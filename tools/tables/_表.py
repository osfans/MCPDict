...
class 表:
	...
	def _正音(自, 音):
		if 自.爲語() and 自.爲音:
			...
			音 = 音.lower().replace("ε", "ɛ").replace("α", "ɑ").replace("g", "ɡ").replace("х", "x").replace("∫", "ʃ").replace("ʼ", "ʰ").replace("'", "ʰ").replace("‘", "ʰ").replace(":","ː").replace("—", "-").replace("－", "-")
			...
			音 = re.sub(r"^([ʔʡˀʕʢbɓɸβʙpmɰɱfʩɟdɗɖᶑʣʤʥꭦðtʈʦʧʨꭧθnŋɲɳȵɴlɬɭʟ𝼄ɮ𝼅ʪʫgɡɠɢʛkʞhħɦɧʰʜzʐʑʒʓcʗçɕsʂrɹɻɺ𝼈ɾɽʀʁʃʄʆjʝʲqxχvʋⱱɣwʍʷʎ𝼆ʼ'ʘǀǃǁǂ𝼊\u0300-\u0362]*)([mnvʋrɹɻlɭŋȵʐɱʒ])(\d+)$", "\\1\\2\u0329\\3", 音)
			音 = re.sub("([mnvʋrɹɻlɭ])([\u0329\u030D]+)", "\\1\u0329", 音)
			音 = re.sub("([ŋȵʐɱʒ])([\u0329\u030D]+)", "\\1\u030D", 音)
			音 = re.sub("^([^*])\\1([^\u0303])", "\\1\\2", 音)
			if 自.無調():
