...
class 表:
	...
	def 校音(自, 音):
		...
		for i in 音.replace("`", "").replace("*", "").split("/"):
			if not IPA_PATTERN.match(i):
	...
	def _正音(自, 音):
		if 自.爲語() and 自.爲音:
			...
			音 = 音.lower().replace("ε", "ɛ").replace("α", "ɑ").replace("g", "ɡ").replace("х", "x").replace("р", "p").replace("∫", "ʃ").replace("ʼ", "ʰ").replace("'", "ʰ").replace("‘", "ʰ").replace(":","ː").replace("—", "-").replace("－", "-")
			...
			音 = 音.replace("ṃ", "m").replace("ḷ", "l")
			音 = re.sub(r"^([ʔʡˀʕʢˤbɓᵇɸβʙpmɰɱᵐfᶠʩɟdɗɖȡᶑᵈʣʤʥꭦðtʈȶᵗʦʧʨꭧθnŋᵑɲɳȵɴlɬɭʟ𝼄ɮ𝼅ʪʫgɡɠᶢɢʛkʞhħɦʱɧʰʜzʐʑʒʓcʗçɕsʂşȿrɹɻɺ𝼈ɾɽʀʁʃʄʆjʝʲqxχˣvʋⱱɣwʍʷʎ𝼆ʼ'~ʘǀǃǁǂ𝼊\u0300-\u0362]*)([mnvʋrɹɻlɭβŋȵʐɱʒ])(\d+)$", "\\1\\2\u0329\\3", 音)
			音 = re.sub("([mnvʋrɹɻlɭ])([\u0329\u030Dˌˈ]+)", "\\1\u0329", 音)
