...
class 表(_表):
	...
	def 析(自, 列):
		...
		調 = ""
		聲韻 = ""
		if 名 in ("蘭州","臨河","包頭","朝邑", "廣靈", "海原", "和順", "鄠縣", "遼縣", "渾源","吉木薩爾","晉城","靜寧城川","酒泉", "藍田", "臨澤", "隴西", '婁煩', '寧武東寨', '平順', '汧陽', '清水', '慶城卅鋪', '山丹', '山陰黑圪塔', '綏德義合', '同心', '烏魯木齊回民', '永寧納家戶回民', '榆社', '正寧榆林子', '中衛', '子洲老君殿', '敏派秦腔', '烏蘇西湖', '翼城蒲劇'):
			if 列[0] == "@TON":
				自.調類調值[列[1]] = 列[2]
		...
		if 自.列序:
			...
			if 名 in ("新望城",):
				...
				組 = 組.replace("｝｛", " ")
				自.爲音 = False
				音 = f"{音}〚{列[2].replace("切", "{切}")}{列[1].replace("韵", "{韻}")}〛"
				組 = 組.replace("｝｛", "")
				...
				音 = 音.replace("平", "1").replace("上", "3").replace("去", "5")
			elif 名 in ("瀘溪武溪","商州羅灣",):
				音 = 音.lstrip("ø").replace("[","").replace("]","")
			elif 名 in ("汨羅",):
				音 = 音.replace("","")
			elif 名 in ("太原","西安","濟南"):
				音 = re.sub(r"..(\d*?)$", "\\1", 音)
