...
class 表(_表):
	...
	def parse(self, fs):
		...
		if name in ("臨川","奉新宋埠"):
			...
			hzs = self.normS(hzs)
		elif name in ("崇陽","通城塘湖","沅陵死客子話","宜章東風客家","新田毛里","資興南鄕", "婁底石井", "雙牌官話", "長沙黎圫","吉首", "懷化", "攸縣新市", "長沙星沙", "東安蘆洪市", "宜章東風", "道縣仙子腳", "臨湘白羊田", "蒲圻羊樓洞", "瀏陽永安","麻坪崇陽", "平江", "資源延東", "祁東白地市", "祁陽", "臨武土地", "樂至南沖寺", "嘉禾普滿", "臨桂陶善"):
			sy, sd, _, hzs = fs[:4]
			hzs = hzs.replace(")(", "；")
			hzs = re.sub(r"(\d)([-=])", "\\2\\1", hzs)
			hzs = self.normS(hzs)
