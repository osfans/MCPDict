...
def 加載(省=None):
	...
	for row in sheet.rows:
		...
		字表格式 = 列["字表格式"]
		if 字表格式 == "縣志" and 字聲韻調註列名: 字表格式 = "跳跳老鼠"
		經緯度 = normJW(語言, 列["經緯度"])
		方言島 = 列["方言島"] == "☑"
		作者 = normNames(列["出處作者(錄入人不是作者)"])
		錄入人 = normNames(列["錄入人"])
		維護人 = normNames(列["維護人"])
		推薦人 = normNames(列["推薦人"])
		...
		for i in ["語言", "地點", "地圖集二分區", "音典分區", "陳邡分區", "歷史音", '方言島', '版本', '作者', '錄入人', '維護人', "發音合作人", '字表修改情況', '字表品質', '字表品質説明', '字表來源', '參考文獻', '補充閲讀']:
			if d[簡稱].get(i):
